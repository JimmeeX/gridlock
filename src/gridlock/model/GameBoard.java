...
public class GameBoard {
	 ...
	 * Added by Edwin
	 */
	public GameBoard duplicateBoard() {
	 ...
	 * Added by Edwin
	 */
	public List<Block> blocksList(int rowOrColumnIndex, boolean isRow) {
		List<Block> particularBlockList = new ArrayList<>();
		for (Block b : this.board.getBlocks()) {
			if (b.isHorizontal() == isRow) {
				if (isRow ? b.getRow() == rowOrColumnIndex : b.getCol() == rowOrColumnIndex)
					particularBlockList.add(b);
			}
		}
		return particularBlockList;
	}
}