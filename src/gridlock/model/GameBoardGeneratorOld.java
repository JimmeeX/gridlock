 ...
 */
public class GameBoardGeneratorOld implements Runnable {
    ...
    public GameBoard generateBoard (GameBoard winBoard, int minMoves, int maxMoves) {
        ...
        */
        ...
        while (!queue.isEmpty()) {
            ...
            for (Block b: curr.board.getBlocks()) {
                ...
                for (int i = intv[0]; i <= intv[1]; i++) {
                    GameBoard duplicate = curr.board.duplicateGridandBlocks();
                    if (b.isHorizontal()) {
        ...
        // Backtracking
        return (minMoves <= maxNode.dist && maxNode.dist <= maxMoves) ? maxNode.board.duplicateGridandBlocks() : null;
    }
